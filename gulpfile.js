...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/angular-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/angular-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && changedFileName !== 'Jenkinsfile' && changedFileName !== 'README.md' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../angular-docs/' + changedFileName)) {
                    ...
                    if (fs.existsSync('../../../gitlabRepo/angular-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../angular-docs/` + changedFileName, `../../../gitlabRepo/angular-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/angular-docs/')) {
                            shelljs.cp('-rf', `../angular-docs/` + changedFileName, `../../../gitlabRepo/angular-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/angular-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/angular-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/angular-docs`);
